...
Fahrbereich: tile_X_0 bis tile_X_7 -> y-Grenze bei +15.0 m
...
#maximal bis tile_X_7 (Mitte y=12.5, Grenze zu _8 bei y=15.0)
# Sicherheitsabstand -0.5 m → Roboter dreht spätestens bei y=14.5
Y_MAX_BOUNDARY   = 14.5   # m – absoluter Y-Stopp (Odometrie-basiert)
    ...
    def loop(self, dt: float):
        ...
        if self._log_timer >= 2.0:
            ...
            self.get_logger().info(
                ...
                f'| Zustand={self.state}  x={self.pos_x:.1f} y={self.pos_y:.1f}')
        ...
        if self.state == STATE_DRIVE:
            ...
            if self.pos_y >= Y_MAX_BOUNDARY:
                ...
                self.get_logger().info(
                    f'Y-Grenze erreicht (y={self.pos_y:.2f}m) – Wende')
                return

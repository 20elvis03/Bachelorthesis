...
STATE_AVOID        = 'AVOID'
STATE_RETURN       = 'RETURN'
STATE_ESCAPE       = 'ESCAPE'        # NEU
STATE_RETURN_BASE  = 'RETURN_BASE'   # NEU
STATE_CHARGING     = 'CHARGING'      # NEU
STATE_DONE         = 'DONE'
STATE_EMERGENCY    = 'EMERGENCY'     # NEU
...
class autodrive(Node):
    ...
    def loop(self, dt: float):
        ...
        if self.obstacle_stop and self.state not in (
                ...
                STATE_RETURN_BASE, STATE_ESCAPE):
            self._publish(0.0, 0.0, 0.0)
        ...
        # NEU: Rückkehr zur Ladestation
            ...
            if self.obstacle_stop:
                side = 1 if self.dist_left >= self.dist_right else -1
                self._publish(AVOID_SPEED*0.5, AVOID_ANGULAR*side, AVOID_STEER*side)
                return
